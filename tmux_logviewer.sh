...
tmux send-keys -t logs:0.0 'docker logs -f execution' Enter
tmux send-keys -t logs:0.1 'docker logs -f validator' Enter
tmux send-keys -t logs:0.2 'docker logs -f beacon' Enter
