...
sudo chown 777 "${custompath}/start_validator.sh"
...
if [[ "$choice" =~ ^[Yy]$ || "$choice" == "" ]]; then
  ...
  command="sudo ${custompath}/./start_execution.sh > /dev/null 2>&1 & sudo ${custompath}/./start_consensus.sh > /dev/null 2>&1 & sudo ${custompath}/./start_validator.sh > /dev/null 2>${custompath}/error.log &"
